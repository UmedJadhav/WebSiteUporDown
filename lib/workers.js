...
workers.rotateLogs = ()=>{
    _logs.list(false,(err,logs)=>{
        if(!err && logs && logs.length > 0){
                    ...
                    if(!err){
                        _logs.truncate(logId,(err)=>{
                            if(!err){
                                ...
                                debug(`Error: Truncation of ${logId}.log File failed`);
                            }
                        });
                    }else{
            ...
            debug('Error:Couldnt find any logs to be rotated');
        }
    });
}
...
workers.processCheckOutcome = (originalCheckData,checkOutcome)=>{
    ...
    _data.update('checks',newCheckData.id,newCheckData,(err)=>{
        if(!err){
            ...
            debug('Error: Failed to save updates to one of the checks');
        }
    }); 
};
...
workers.alertUsersToStatusChange = (newCheckData)=>{
    ...
    helpers.sendTwilioSms(newCheckData.userPhone,message,(err)=>{
        if(!err){
            debug(`Success: User ${newCheckData.userPhone} was alerted to a status Change of ${newCheckData.state} ` , message);
        }else{
            debug(`Error: User ${newCheckData.userPhone} couldnt be notified of state change alert `);
        }
    });
};
